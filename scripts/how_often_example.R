...
labels.x <- paste(seq(0.00, 0.4, 0.005),"%",sep = "")
...
ggplot(data = df, aes(x = ring, y = mine, label = char)) +
  ...
  annotate("label", x = 0.025, y = 0.018, label = '¯¹dni pierœcienia', size = 10, color = 'red') + 
  annotate('label', x = 0.028, y = 0.003, label = 'Zainteresowani pierœcieniem', size = 10, color = 'orange') +
  ...
  scale_x_continuous(breaks = seq(0.00, 0.4, 0.005), labels = labels.x) +
